...
def finalize_prompt_logits(receiver_model, cache, input_ids: torch.Tensor, attention_mask: torch.Tensor | None = None) -> PartialPrefillResult:
    ...
    if attention_mask is not None and prefix_layers:
        prefix_length = next(iter(prefix_layers.values()))[0].shape[-2]
        expected_mask_length = prefix_length + 1
        if attention_mask.shape[-1] != expected_mask_length:
            attention_mask = attention_mask[:, -expected_mask_length:]
