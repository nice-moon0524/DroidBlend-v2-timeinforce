...
class PartialDynamicCache(DynamicCache):
    ...
    def update(self, key_states, value_states, layer_idx, cache_kwargs=None):
        ...
        if not layer.is_initialized:
            # Transformers changed lazy_initialization from one argument to
            # (key_states, value_states). Support both APIs.
            try:
                layer.lazy_initialization(key_states, value_states)
            except TypeError:
                layer.lazy_initialization(key_states)
            layer.keys = torch.empty_like(key_states[..., :0, :])
            layer.values = torch.empty_like(value_states[..., :0, :])
